...
- `HyperSoundEngineRust/` — **Rust 支线**：`hse-core` 已完成 17 个 DSP 模块与 `EngineChainStage` 1–21 级主链，音频冻结向量 **72/72 PASS**；`hrtf-core` 已完成 world-listener position/yaw 几何核并通过空间夹具 **12/12 PASS**，尚不含 HRIR/卷积/房间渲染；`hse-parity` 为综合共享规格门禁；`hse-wasapi` 提供 WASAPI 共享模式渲染 + loopback 捕获（`wasapi` crate 精确锁版 `=0.24.0`）；`hse-service` 提供捕获→DSP→渲染三线程、rtrb 双环、localhost WebSocket JSON-RPC（默认 `ws://127.0.0.1:4780/`）与推流协议；`hse-wasm` 已完成单 Biquad + 独立 AudioWorklet 的 wasm32 最小试点，不是完整引擎替换；`hse-napi` 仍为占位，未入 workspace members。Windows 音频后端固定为 WASAPI，不提供 MIDI 或 ASIO
