...
4. **对拍冻结约束是硬边界**（§四）：容差公式 `|got−want| ≤ 1e-6 × max(|want|, 1e-9)`
   ...
   **只允许"逐元素保序"的 SIMD**。
5. **8h soak 本会话无法执行**（时间预算 + 真实设备拓扑依赖），短窗证据为
   Phase 2 验收 14/14 + 5 分钟 soak；正式 8h 长跑的执行清单见 §七。
...
- **§三指标全绿**：全链默认快照 0.546% realtime（60s 音频 327.7ms，113.8 ns/帧）
...
### 7.4 8 小时 soak：现状与执行清单

**本会话无法执行 8h soak**（会话时间预算 + 需要真实音频设备拓扑，且本任务
约束禁止接入音频设备）。已有短窗证据（引用，不重复）：

- Phase 2 真机端到端验收 **14/14 PASS**（[`service-phase2-acceptance.md`](service-phase2-acceptance.md)）：
  真实音频流经全链、计数器单调、xrun 上报链路正确、停止/启动全序；
- **5 分钟 soak**（同文件 §五）：60s 间隔采样 ×5，phase 恒 running、
  framesProcessed 持续增长、计数器单调；
- 已知伪影（同文件 §四）：xrunsOut≈4.7s 当量归因于 SoundPlayer 供块抖动 +
  同设备回授，非服务缺陷——这正是 8h 长跑要换拓扑的原因。

**正式 8h run 的执行要件**（留档待后续轮次）：

1. **拓扑**：异设备回放（或安装 VB-CABLE 虚拟缆直捕；本机未装），消除同设备
   回授；正式播放器（foobar2000/浏览器）或 push-stream 协议脚本化推流
   （`specs/service/push-stream.md`）作为稳定源；
2. **采样**：≤60s 间隔 `getState` 轮询——framesProcessed 严格递增且斜率稳定
   （漂移检测）、xrun 计数（目标 8h = 0）、phase 恒 running、RSS 曲线（平台期
   判定零泄漏；convolver `pending` 突发扩容是唯一潜在增长点，输入块长 ≤Ls 时
   不触发）；
3. **负载**：建议跑最重稳态场景（全开 + 长 IR 卷积，§三口径 10.7% 单核）与
   默认场景各一轮，覆盖 GC 无、分配无（稳态零分配铁律）的长期证据；
4. **通过判据**：8h 零 xrun、无 phase 回退、framesProcessed 斜率漂移 <1%、
   RSS 平台期、末段对拍重放逐位一致（reset 重放性质）；
5. **时间预算**：8h 墙钟 × 2 场景 + 分析，独立会话执行；本文件不承担。
