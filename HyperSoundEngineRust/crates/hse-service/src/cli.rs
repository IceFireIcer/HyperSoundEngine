...
pub fn run(args: impl Iterator<Item = String>) -> i32 {
    ...
    let url = url.or_else(|| std::env::var("HSE_SERVICE_URL").ok())
...
fn build_configure(rest: &[String]) -> Result<Value, String> {
    ...
    while idx < rest.len() {
        match rest[idx].as_str() {
            "--device" => {
                idx += 1;
                device = Some(rest.get(idx).ok_or("--device 缺少取值")?.clone());
            }
            "--rate" => {
                idx += 1;
                rate = rest.get(idx).ok_or("--rate 缺少取值")?.parse().map_err(|_| "--rate 需要整数")?;
            }
            "--block" => {
                idx += 1;
                block = rest.get(idx).ok_or("--block 缺少取值")?.parse().map_err(|_| "--block 需要整数")?;
            }
...
fn request(ws: &mut CliWs, method: &str, params: Value) -> Result<Value, String> {
    ...
    ws.send(Message::text(line)).map_err(|e| format!("发送失败：{}", e))?;
    loop {
...
fn print_devices(result: &Value, full: bool) {
    for (label, key) in [("渲染设备 (render)", "render"), ("捕获设备 (capture)", "capture")] {
        ...
        for d in result[key].as_array().unwrap_or(&empty) {
            ...
            println!("  {} {}  {}{}{} {}", mark, YELLOW, d["name"], RESET, DIM, id_text);
            if !full {
