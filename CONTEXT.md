...
- **音频入口 (Audio Ingress)**：播放器音频进入引擎服务进程的通道，两种形态并存：
  - **回环拦截 (Loopback Intercept)**：引擎捕获指定输出设备（或虚拟音频缆）的 WASAPI loopback 混音。接入方零集成——把输出设备选成虚拟缆即可。
  - **推流 (Push Stream)**：接入方自行解码，按引擎服务进程的音频流协议推送 PCM 块；每条流是一个独立**会话 (Session)**。
- **渲染输出 (Render Output)**：引擎处理后音频的最终出口（WASAPI / ASIO 设备）。回环拦截仅支持 WASAPI；推流与 ASIO 兼容。
